...
//GaneData, Item 클래스가 담긴 네임스페이스 명시
using DataInfo;
public class GameManager : MonoBehaviour
{
    ...
    //적 캐릭터의 프리팹을 저장할 변수
    public GameObject enemy; //프리팹을 저장할때는 게임오브젝트 타입으로 선언한다.
    //적 캐릭터를 생성할 주기
    ...
    //싱글턴 : 객체를 하나로 유지된다 (ex: 휴대폰의 볼륨)
    //static이기때문에 어디서든 참조값 없이 접근할 수 있다.
    public static GameManager instance = null;

    public const string kKillCount = "KILL_COUNT";
    [Header("Object Pool")]
    //생성할 총알 프리팹
    ...
    //주인공이 죽인 적 캐릭터의 수
    //인스펙터를 통해 값을 변경할수 없게하고 다른객체가 접근할수 있도록 한것
    ...
    private void Awake()
    {
        ...
        //매 스테이지마다 데이터를 유지시키기 위해 사용
        //안쓰게 되면 1번스테이지것들이 2번스테이지로 넘어가면 사라지기 때문에 유지될것은 이 코드를 사용한다.
    ...
    void InventorySetup()
    {
        ...
        for (int i=0; i< gameData.equipItem.Count; i++)
        {
            ...
            for (int j = 1; j < slots.Length; j++) //1부터인 이유는 부모인 SlotList객체포함이기 때문에
            {
        ...
        */
        ...
        for (int i = 0; i < gameData.equipItem.Count; i++)
        {
            int slotIndex = gameData.equipItem[i].slotIndex;

            if (slotIndex < 0 && slots[slotIndex].childCount > 0) continue;

            // 보유한 아이템의 종류에 따라 인덱스를 추출
            int itemIndex = (int)gameData.equipItem[i].itemType;

            // 아이템의 부모를 Slot 게임오브젝트로 변경
            itemObjects[itemIndex].GetComponent<Transform>().SetParent(slots[slotIndex + 1]);
            // 아이템의 ItemInfo 클래스의 itemData에 로드한 데이터 값을 저장
            itemObjects[itemIndex].GetComponent<ItemInfo>().itemData = gameData.equipItem[i];
        }
    }
    ...
    void Start()
    {
        ...
        //하이러키 뷰의 SpanPointGroup을 찾아 하위에 있는 모든 Transform 컴포넌트를 찾아옴
    ...
    IEnumerator CreateEnemy()
    {
        ...
        while (!isGameOver)
        {
            ...
            {
                //적 캐릭터의 생성 주기 시간만큼 대시
                yield return new WaitForSeconds(createTime);

                //불규칙적인 위치 산출
                //1부터인 이유는 GameObject.Find("SpawnPointGroup")로 SpawnPointGroup도 찾아서 0번에 들어가기 때문이다.
